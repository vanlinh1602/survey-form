...
export default function InputPage({ reportId }: Props) {
  ...
  const { info: userInfo, form: currentForm } = allReports?.[reportId] || {};

  const { title, data: formSchema } = resources[userInfo?.level || 'mn'] ?? {
    title: '',
    data: {},
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setReport(reportId, { form: formData });
